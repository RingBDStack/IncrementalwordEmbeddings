...
The training algorithms were originally ported from the C package https://model.google.com/p/word2vec/
...
The word vectors are stored in a KeyedVectors instance in model.wv. This separates the read-only word vector lookup operations in KeyedVectors from the training model in Word2Vec.
...
from numpy import dot, zeros, outer, random, dtype, float32 as REAL, \
    ...
    def train_batch_sg(model, sentences, alpha, work=None):
        ...
        for sentence in sentences:
            ...
            for pos, word in enumerate(word_vocabs):
                reduced_window = model.random.randint(model.window)  # `b` in the original word2vec model
...
def train_sg_pair_new(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    if model.hs:
        ...
        l2a = deepcopy(model.syn1[predict_word.point[predict_word.max_common + 1:]])  # 2d matrix, codelen x layer1_size
        fa = expit(dot(l1, l2a.T))  # propagate hidden -> output
        ga = (1 - predict_word.code[
                  predict_word.max_common + 1:] - fa) * alpha  # vector of error gradients multiplied by the learning rate
        if learn_hidden:
            model.syn1[predict_word.point[predict_word.max_common + 1:]] += outer(ga, l1)  # learn hidden -> output
            # count gradient updates '+'
            for i in range(predict_word.max_common + 1, len(predict_word.point)):
                layer = i
                model.update_gradient_count['+'][layer] += 1
            # Undo Learn weights hidden -> output
            model.syn1[predict_word.old_neurons] -= outer(ga, l1)[:len(predict_word.old_neurons)]
            # count gradient updates '-'
            for i in range(len(predict_word.old_neurons)):
                layer = i + predict_word.max_common
                model.update_gradient_count['-'][layer] += 1
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
                  context_vectors=None, context_locks=None):
    if model.update:
        return train_sg_pair_new(model, word, context_index, alpha, learn_vectors,
                                 learn_hidden, context_vectors, context_locks)
    if context_vectors is None:
        context_vectors = model.wv.syn0
    if context_locks is None:
        context_locks = model.syn0_lockf
...
def train_cbow_pair_new(model, word, input_word_indices, l1, alpha, learn_vectors=True, learn_hidden=True):
    neu1e = zeros(l1.shape)
    if model.hs:
        l2a = model.syn1[word.point[word.max_common + 1:]]  # 2d matrix, codelen x layer1_size
        fa = expit(dot(l1, l2a.T))  # propagate hidden -> output
        ga = (1. - word.code[
                   word.max_common + 1:] - fa) * alpha  # vector of error gradients multiplied by the learning rate
        if learn_hidden:
            model.syn1[word.point[word.max_common + 1:]] += outer(ga, l1)  # learn hidden -> output
            # count gradient updates '+'
            for i in range(word.max_common + 1, len(word.point)):
                layer = i
                model.update_gradient_count['+'][layer] += 1
            # Undo Learn weights hidden -> output
            model.syn1[word.old_neurons] -= outer(ga, l1)[:len(word.old_neurons)]
            # count gradient updates '-'
            for i in range(len(word.old_neurons)):
                layer = i + word.max_common
                model.update_gradient_count['-'][layer] += 1
        neu1e += dot(ga, l2a)  # save error
...
def train_cbow_pair(model, word, input_word_indices, l1, alpha, learn_vectors=True, learn_hidden=True):
    if model.update:
        return train_cbow_pair_new(model, word, input_word_indices, l1, alpha, learn_vectors, learn_hidden)
...
class Word2Vec(utils.SaveLoad):
    """
    Class for training, using and evaluating neural networks described in https://model.google.com/p/word2vec/
    ...
    def __init__(
        ...
        texts are longer than 10000 words, but the standard cython model truncates to that maximum.)
    ...
    def create_binary_tree(self, update=False):
        """
        ...
        for i in xrange(len(self.wv.vocab) - 1):
            ...
            heapq.heappush(heap, Vocab(weight=min1.weight + min2.weight, index=i + len(self.wv.vocab), left=min1,
                                       right=min2))

        # recurse over the tree, assigning a binary model to each vocabulary word
        if heap:
            ...
            self.heap = heap
            self.update_gradient_count = {"+": array([0 for _ in range(max_depth)], dtype=uint32),
                                          "-": array([0 for _ in range(max_depth)], dtype=uint32)}
            logger.info("built a tree with maximum node depth %i", max_depth)
    ...
    def build_vocab(self, sentences, keep_raw_vocab=False, trim_rule=None, progress_per=10000, update=False):
        ...
        self.set_word_weight(sentences)
    ...
    def scale_vocab(self, min_count=None, sample=None, dry_run=False, keep_raw_vocab=False, trim_rule=None,
        ...
        if not update:
            ...
            # for each exist word record its path and point in old model
            for word, leaf in self.wv.vocab.items():
                leaf.old_code = leaf.code
                leaf.old_point = leaf.point
                leaf.count = 0
            ...
            # set new word's old_code and old_point
            for word in set(new_words):
                leaf = self.wv.vocab[word]
                leaf.old_code = array([], dtype=uint8)
                leaf.old_point = array([], dtype=uint32)
    ...
    def finalize_vocab(self, update=False):
        ...
        if self.hs:
            ...
            self.create_binary_tree(update)
        if self.negative:
    ...
    def set_word_weight(self, sentences, window=2):
        word_weight = {}
        for sentence_no, sentence in enumerate(sentences):
            ...
            for word_index, word in enumerate(sentence):
                if word in self.wv.vocab:
                    if word not in word_weight:
                        word_weight[word] = {}
                    for i in range(max(0, word_index - window), min(len_sent, word_index + window)):
                        context_word = sentence[i]
                        if context_word not in word_weight[word]:
                            if context_word in self.wv.vocab:
                                word_weight[word][context_word] = 1
                        else:
                            word_weight[word][context_word] += 1

        for word in word_weight:
            if hasattr(self.wv.vocab[word], "context"):  # exist word
                for context_word in word_weight[word]:
                    if context_word in self.wv.vocab[word].context:
                        self.wv.vocab[word].context[context_word] += word_weight[word][context_word]
                    else:
                        self.wv.vocab[word].context[context_word] = word_weight[word][context_word]
            else:   # new word
                self.wv.vocab[word].context = word_weight[word]

            # compute weight
            context = self.wv.vocab[word].context
            count = [context[context_word] for context_word in context]
            weight = [self.wv.vocab[context_word].count for context_word in context]
            count_sum = sum(count)
            freq = [float(i) / count_sum for i in count]

            weight = sum([weight[i] * freq[i] for i in range(len(count))])

            # set weight
            self.wv.vocab[word].weight = weight

    def sort_vocab(self):
        """Sort the vocabulary so the most frequent words have the lowest indexes."""
        if len(self.wv.syn0):
            raise RuntimeError("must sort before initializing vectors/weights")
        self.wv.index2word.sort(key=lambda word: self.wv.vocab[word].weight, reverse=True)
    ...
    def train(self, sentences, total_words=None, word_count=0,
              total_examples=None, queue_factor=2, report_delay=1.0, update=False):
        """
        ...
        self.clear_sims()
        out_s = " ".join([str(c) for c in self.update_gradient_count["-"]])
        logging.info("-:" + out_s)
        out_s = " ".join([str(c) for c in self.update_gradient_count["+"]])
        logging.info("+:" + out_s)
    ...
    def update_para(self):
        logger.info("inherit neurons")
        self.old_syn1 = self.syn1

        self.update_weights()

        total_words = len(self.wv.vocab)
        inherit_set = set([])

        for word in self.wv.vocab:
            word = self.wv.vocab[word]
            if len(word.old_code) > 0:
                for i in range(min(len(word.old_code), len(word.code))):
                    if word.old_code[i] == word.code[i]:
                        # inherit common syn1
                        self.syn1[word.point[i]] = self.old_syn1[word.old_point[i]]
                        inherit_set.add(word.point[i])
                    else:
                        break
                self.syn1[word.point[i]] = self.old_syn1[word.old_point[i]]
                inherit_set.add(word.point[i])
                word.max_common = i     # max_common is greater than 0 for sure

        for word in self.wv.vocab:
            word = self.wv.vocab[word]
            if len(word.old_code) > 0:
                # find first different point
                first_dif_point = self.heap[0]
                for i in range(0, word.max_common + 1):
                    if word.old_code[i] == 0:
                        first_dif_point = first_dif_point.left
                    elif word.old_code[i] == 1:
                        first_dif_point = first_dif_point.right

                # add point which is in old path and not in new path into word.old_neurons
                diff_point = first_dif_point
                word.old_neurons = []
                for i in range(word.max_common, min(len(word.old_code), len(word.code)) - 1):
                    # find point in old path
                    if hasattr(diff_point, 'left'):     # is not a leaf
                        hidden_index = diff_point.index - total_words
                        # add to word.old_neurons
                        if hidden_index in inherit_set:
                            word.old_neurons.append(hidden_index)
                        else:
                            break

                        if word.old_code[i] == 0:
                            diff_point = diff_point.left
                        elif word.old_code[i] == 1:
                            diff_point = diff_point.right
                    else:
                        break
                word.old_neurons = array(word.old_neurons, dtype=uint32)
            else:
                word.max_common = -1
                word.old_neurons = array([], dtype=uint32)
        logger.info("inherit hidden neurons:" + str(len(inherit_set)))
        self.heap = []
        self.inherit_set = inherit_set
