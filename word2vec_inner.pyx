...
# incremental train
cdef void fast_sentence_cbow_hs_incremental(
    const np.uint32_t *word_point, const np.uint8_t *word_code, int codelens[MAX_SENTENCE_LEN], int old_codelens[MAX_SENTENCE_LEN],
    REAL_t *neu1, REAL_t *syn0, REAL_t *syn1, const int size,
    const np.uint32_t indexes[MAX_SENTENCE_LEN], const REAL_t alpha, REAL_t *work,
    int i, int j, int k, int cbow_mean, REAL_t *word_locks, int max_common, np.uint32_t *old_point, np.uint32_t *gradient_count_plus, np.uint32_t *gradient_count_minus)nogil:

    cdef long long a, b, c
    cdef long long row2, row3
    cdef REAL_t f, g, count, inv_count = 1.0
    cdef int m
    ...
    memset(work, 0, size * cython.sizeof(REAL_t))
    for b in range(max_common+1, codelens[i]):  # neu1 = syn0
        row2 = word_point[b] * size
        f = our_dot(&size, neu1, &ONE, &syn1[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
        our_saxpy(&size, &g, &syn1[row2], &ONE, work, &ONE) # work += g * syn1
        our_saxpy(&size, &g, neu1, &ONE, &syn1[row2], &ONE) # syn1 += g * syn0
        gradient_count_plus[b] += 1
        # Undo Learn weights hidden -> output
        c = b - max_common - 1
        if c < old_codelens[i]:
            row3 = old_point[c] * size
            g *= -1
            our_saxpy(&size, &g, neu1, &ONE, &syn1[row3], &ONE) # syn1 += -g * syn0
            gradient_count_minus[b] += 1
...
cdef void fast_sentence_sg_hs_incremental(
    const np.uint32_t *word_point, const np.uint8_t *word_code, const int codelen,
    REAL_t *syn0, REAL_t *syn1, const int size,
    const np.uint32_t word2_index, const REAL_t alpha, REAL_t *work, REAL_t *word_locks,
    int max_common, int old_codelens, np.uint32_t *old_point, np.uint32_t *gradient_count_plus, np.uint32_t *gradient_count_minus) nogil:

    cdef long long a, b
    cdef long long row1 = word2_index * size, row2, row3
    cdef REAL_t f, g

    memset(work, 0, size * cython.sizeof(REAL_t))
    for b in range(max_common+1, codelen):
        row2 = word_point[b] * size
        f = our_dot(&size, &syn0[row1], &ONE, &syn1[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        ...
        if c < old_codelens:
            row3 = old_point[c] * size
            g *= -1
            our_saxpy(&size, &g, &syn0[row1], &ONE, &syn1[row3], &ONE) # syn1 += -g * syn0
            gradient_count_minus[b] += 1
    our_saxpy(&size, &word_locks[word2_index], work, &ONE, &syn0[row1], &ONE)
...
def train_batch_sg(model, sentences, alpha, _work):
    ...
    cdef int max_common[MAX_SENTENCE_LEN]
    cdef int old_codelens[MAX_SENTENCE_LEN]
    cdef np.uint32_t *old_points[MAX_SENTENCE_LEN]
    cdef np.uint32_t *gradient_count_minus
    cdef np.uint32_t *gradient_count_plus
    if model.update:
        gradient_count_minus = <np.uint32_t *>np.PyArray_DATA(model.update_gradient_count["-"])
        gradient_count_plus = <np.uint32_t *>np.PyArray_DATA(model.update_gradient_count["+"])
    # end
    ...
    for sent in sentences:
        ...
        for token in sent:
            ...
            if hs:
                ...
                if update:
                    old_points[effective_words] = <np.uint32_t *>np.PyArray_DATA(word.old_neurons)
                    max_common[effective_words] = <int>word.max_common
                    old_codelens[effective_words] = <int>len(word.old_neurons)
            effective_words += 1
        ...
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                ...
                for j in range(j, k):
                    ...
                    if hs:
                        if update:
                            fast_sentence_sg_hs_incremental(points[i], codes[i], codelens[i], syn0, syn1, size, indexes[j], _alpha, work, word_locks, max_common[i], old_codelens[i], old_points[i], gradient_count_plus, gradient_count_minus)
                        else:
    ...
    with nogil:
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                ...
                if hs:
                    if update:
                        fast_sentence_cbow_hs_incremental(points[i], codes[i], codelens, old_codelens, neu1, syn0, syn1, size, indexes, _alpha, work, i, j, k, cbow_mean, word_locks, max_common[i], old_points[i], gradient_count_plus, gradient_count_minus)
                    else:
